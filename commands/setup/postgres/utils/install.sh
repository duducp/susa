...
install_postgres_macos() {
    ...
    if ! command -v ${POSTGRES_UTILS[0]} &> /dev/null; then
        ...
        log_debug "Executando: brew link --force $POSTGRES_CLIENT_PKG_HOMEBREW"
        if ! brew link --force "$POSTGRES_CLIENT_PKG_HOMEBREW" 2>&1 | while read -r line; do log_debug "brew: $line"; done; then
